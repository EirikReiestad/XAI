...
class CoopEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        terminated = self.agents.colliding
        collided = False
        ...
        reward = self.coop_rewards.get_reward(
    ...
    def _move_agent_within_bounds(
        self, state: np.ndarray, agent: Position
    ) -> Optional[np.ndarray]:
        state[*self.agents.active.position] = TileType.EMPTY.value
        self.agent = agent
        state[*self.agents.active.position] = AGENT_TILE_TYPE[self.agents.active_agent]
        return state
