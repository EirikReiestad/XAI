    ...
    if isinstance(space, Box):
        if space.dtype.kind == "f":
            ...
            raise NotImplementedError(
                f"Unknown test for Box(dtype={space.dtype})")
        ...
        variance = (
            2 * np.square(observed_frequency -
        ...
        observed_frequency = _generate_frequency(
            space.nvec, lambda dim: np.zeros(dim))
        for sample in samples:
            _update_observed_frequency(
                sample - space.start, observed_frequency)
        ...
        assert observed_frequency.shape == expected_frequency.shape
        assert np.sum(observed_frequency) == sum(len(sample)
                                                 for sample in samples)
...
def chi2_test(sample, low, high, bounded_below, bounded_above):
    ...
    if bounded_below and bounded_above:
        ...
        observed_frequency = np.bincount(
            sample - low, minlength=degrees_of_freedom)
        assert observed_frequency.shape == (degrees_of_freedom,)
        expected_frequency = np.ones(
            degrees_of_freedom) * n_trials / degrees_of_freedom
    elif bounded_below and not bounded_above:
        # X ~ low + Geom(1 - e^-1)
        # => X - low ~ Geom(1 - e^-1)
...
def test_space_sample_mask(space: Space, mask, n_trials: int = 100):
    ...
    if isinstance(space, Discrete):
        if np.any(mask == 1):
            expected_frequency = np.ones(
                space.n) * (n_trials / np.sum(mask)) * mask
        else:
        ...
        if degrees_of_freedom == 0:
            ...
            assert variance < scipy.stats.chi2.isf(
                ALPHA, df=degrees_of_freedom)
    elif isinstance(space, MultiBinary):
        expected_frequency = (
        ...
        _chi_squared_test(space.nvec, mask,
                          expected_frequency, observed_frequency)
    elif isinstance(space, Text):
